...
#define ACCESS_POINT_NAME      "Somfy-Dongle"  // default SSID for Admin-Mode
...
boolean ReadConfig()
{
  ...
  { // upgrade config to version 2
    config.mqtt_devicetopic = "somfy"; // default devicetopic
    config.cfgVersion = 2;
    // clear EEPROM space
...
void InitializeConfigData()
{
  ...
  if (config.mqtt_broker_client_id == "SomfyDongle") {
    config.mqtt_broker_client_id += chipIdString;
